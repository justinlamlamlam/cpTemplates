...
ll segtreequery(ll v, ll tl, ll tr, ll l, ll r){
    ...
    ll tm = (tl + tr) / 2;
    return __gcd(segtreequery(v*2, tl, tm, l, min(r, tm)) , segtreequery(v*2+1, tm + 1, tr, max(l, tm+1), r));
}
...
int main(){
    ...
    for(int i = 0;i<n;i++){
        g[i] = 1<<i;
        updatesegtree(1, 0, n-1, i, 1<<i);
    }
    for(int i = 0;i<n;i++){
        for(int j = i;j<n;j++){
            cout<<"gcd from "<<i<<" to "<<j<<" = "<<segtreequery(1, 0, n-1, i, j)<<"\n";
        }
    }
    return 0;
}
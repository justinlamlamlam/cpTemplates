...
int main(){
    ...
    cin.tie(NULL);
    //incomplete, not sure how to use
    ll n;
    cin>>n;
    graph g(n), edgeid(n);
    for(int i = 0;i<n-1;i++){
    ...
    for(int i = 0;i<q;i++){
        ...
        if(c == 1){
            ...
            cin>>x;
            bool start = false;
            //request painting edge x;
            //if start = true, then the edge is painted, otherwise the painting is removed
            if(!edge_used[x]){
                start = true;
            }
            edge_used[x] = !edge_used[x];
            sum_tree_update(tree1, 1, 0, (ll)edges_list.size() - 1, first1[x], start ? 1 : -1);
            sum_tree_update(tree2, 1, 0, (ll)edges_list.size() - 1, first2[x], start ? 1 : -1);
        }
        else{
            ll v1,v2;
            cin>>v1>>v2;
            ll l = lca(v1, v2);
            ll result = query(l, v1) + query(l, v2);
            cout<<result<<endl;
        }
    }
    return 0;
